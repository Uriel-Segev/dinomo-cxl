...
void run(unsigned thread_id, Address ip, vector<Address> monitoring_ips) {
  ...
  //int hwm_inb_msg = 0;
  //key_address_puller.setsockopt(ZMQ_RCVHWM, &hwm_inb_msg, sizeof(hwm_inb_msg));
...
int main(int argc, char *argv[]) {
  ...
  unsigned kStorageThreadCount = threads["storage"].as<unsigned>();
  ...
  unsigned kDefaultGlobalStorageReplication = replication["storage"].as<unsigned>();
  ...
  kTierMetadata[Tier::STORAGE] =
      TierMetadata(Tier::STORAGE, kStorageThreadCount, kDefaultGlobalStorageReplication,
                   kStorageNodeCapacity);

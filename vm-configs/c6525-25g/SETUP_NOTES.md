...
## Hardware
- Node: CloudLab c6525-25g allocation (the experiment hostname changes per allocation)
- CPU: AMD EPYC 7302P, 32 cores (16 physical, SMT)
- RAM: 125GB
- Disk: /dev/sda (447GB), /dev/sdb (447GB)
  - `/dev/sda` contains the host OS
  - `setup_vms.sh` formats the empty `/dev/sdb` scratch disk and mounts it at `/mnt/data` for VM images
- NICs:
...
sudo rdma link add rxe0 type rxe netdev enp2s0   # enp2s0 = br-rdma interface
...
## Scripts
- setup_vms.sh — prepare the host, create all 5 VMs, install dependencies, and build DINOMO
- start.sh — configure Soft-RoCE and start all components in order
- test.sh — run the LOAD, WRITE, READ, and MIXED benchmark sequence
...
2. Single host — all VMs run on the current CloudLab experiment node
